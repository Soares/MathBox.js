...
a.x+(d.x-a.x)*h,f.y=a.y+(d.y-a.y)*h,f.z=a.z+(d.z-a.z)*h,f):d.constructor==THREE.Color?(f=new THREE.Color,f.r=a.r+(d.r-a.r)*h,f.g=a.g+(d.g-a.g)*h,f.b=a.b+(d.b-a.b)*h,f):0.5<j?d:a}}this.start||this.init(a);var c=this.object;a=this.from;var d=this.to,e=this.key,f=this.ease,j;this.fraction=j=0<this.duration?Math.min(1,(MathBox.Animator.now-this.start)/(this.duration||1)):1;var h;switch(f){case "in":h=1-(1-j)*(1-j);break;case "out":h=j*j;break;case "linear":h=j;break;default:h=0.5-0.5*Math.cos(j*\u03c0)}a=
