...
m=b.tickScale,b=b.decimals,l=this.points,q=this.labelPoints,n=this.labelTangent,r=this.tickPoints,s=this.tickSigns,t=[0,0,0],u=new THREE.Vector3;new THREE.Vector4;a=a.axis(c);var v=a[0];a=a[1];var x=(a-v)/(h-1);u.set.apply(u,d);_.loop(h,function(a){t[c]=v+a*x;l[a].set.apply(l[a],t);l[a].addSelf(u)});this.line.show(f);this.arrow.show(e);h=this.scale=MathBox.Ticks(v,a,g,k,m,!0);this.ticks.show(!!g);if(g){g=Math.floor(r.length/2);h.length>g&&(h=h.slice(0,g));var g=[0,0,0],w;g[2==c?0:1-c]=0.01*(a-v);
this.epsilon.set.apply(this.epsilon,g);t=[0,0,0];_.each(h,function(a,b){t[c]=a;var d=2*b;r[d].set.apply(r[d],t);r[d].addSelf(u);s[d]=1;r[d+1].copy(r[d]);s[d+1]=-1;q[b].copy(r[d]);w=d+1});g=w+1;for(h=r.length;g<h;)r[g].copy(r[w]),s[g]=s[w],g++}t=[0,0,0];t[c]=1;n.set.apply(n,t);this.labels.show(!!j);this.labels.set("decimals",b)},make:function(){var a=this.get(),b=a.n,c=a.size,d=a.ticks,e=a.distance,f=a.formatter,j=this.style,h=this.points=[],g=this.labelPoints=[],k=this.labelTangent=new THREE.Vector3,
m=this.tickPoints=[],l=this.tickSigns=[],q=this.epsilon=new THREE.Vector3;_.loop(b,function(){h.push(new THREE.Vector3)});_.loop(4*d,function(){g.push(new THREE.Vector3)});_.loop(8*d,function(){m.push(new THREE.Vector3);l.push(1)});var d={dynamic:!0,size:c,offset:0.5},c={dynamic:!0,size:0.2*c},e={dynamic:!0,distance:e},n=function(b){b=this.scale[b];return 0==b&&!a.zero?"":f?f(b):b}.bind(this);this.line=new MathBox.Renderable.Mesh(h,{dynamic:!0,type:"line"},j);this.arrow=new MathBox.Renderable.ArrowHead(h[b-
2],h[b-1],d,j);this.ticks=new MathBox.Renderable.Ticks(m,l,q,c,j);this.labels=new MathBox.Renderable.Labels(g,k,n,e,j)}});MathBox.Axis.validateArgs=function(a){"number"==typeof a&&(a={axis:a});return a};MathBox.Primitive.types.axis=MathBox.Axis;MathBox.Grid=function(a){null!==a&&MathBox.Primitive.call(this,a)};
MathBox.Grid.prototype=_.extend(new MathBox.Primitive(null),{defaults:function(){return{axis:[0,1],offset:[0,0,0],show:[!0,!0],n:2,ticks:[10,10],tickUnit:[1,1],tickScale:[10,10],style:{lineWidth:1,color:new THREE.Color(10526880)}}},type:function(){return"grid"},renderables:function(){return this.lines||[]},adjust:function(a){function b(a,b,c,d,e){var f=k.slice(),g=0,h=MathBox.Ticks(b.min,b.max,b.ticks,b.tickUnit,b.tickScale,!0);h.length>d&&(h=h.slice(0,d));_.each(h,function(d){f[b.axis]=d+k[b.axis];
...
m=new THREE.Vector3,l=new THREE.Vector3,q=0,n=0;_.loop(2*h,function(h){p=f&&void 0!==f[h]?f[h]:e.expression.call(this,n,q);p instanceof Array||(p=[h,p,0]);p=p.concat([0,0,0]);d[h].set.apply(d[h],p);b[h].set.apply(b[h],p);if(a&&j&&1==h%2){l.copy(b[h]);m.copy(b[h-1]);a.to(l);a.to(m);l.subSelf(m).multiplySelf(k);var s=l.length(),t=Math.min(1,0.5*s/g),t=(1-(1-t)*(1-t))*g;l.normalize().multiplyScalar(s-t).divideSelf(k).addSelf(m);a.from(l);b[h].copy(l);c[n].set({size:t})}n=q?n+1:n;q=(q+1)%2}.bind(this))}});
...
{type:"points",dynamic:a.live},b);if(a.shaded){var e=this.tangents=[[],[]];_.loop(c[0]*c[1],function(){e[0].push(new THREE.Vector3);e[1].push(new THREE.Vector3)})}this.calculate()},calculate:function(){var a=this.vertices,b=this.tangents,c=this.get(),d=c.data,e=c.domain,f=c.n,j=c.flipSided,h=j?1:0,j=j?0:1;"number"==typeof f&&(f=[f,f]);var g=e[0][0],k=e[1][0],m=(e[0][1]-g)/(f[0]-1),l=(e[1][1]-k)/(f[1]-1),q,n=0;_.loop(f[1],function(b){g=e[0][0];_.loop(f[0],function(e){q=d&&void 0!==d[b]&&void 0!==d[b][e]?
d[b][e]:c.expression.call(this,g,k,e,b,n);q instanceof Array||(q=[g,q,k]);q=q.concat([0,0,0]);a[n].set.apply(a[n],q);g+=m;n++}.bind(this));k+=l}.bind(this));if(c.shaded){var n=0,k=e[1][0],r=f[0];_.loop(f[1],function(c){g=e[0][0];var d=c?c-1:0,h=Math.min(f[1]-1,c+1);_.loop(f[0],function(e){var j=e?e-1:0,k=Math.min(f[0]-1,e+1),l=a[e+c*r];k==e?b[0][n].sub(l,a[j+c*r]).addSelf(l):b[0][n].copy(a[k+c*r]);h==c?b[1][n].sub(l,a[e+d*r]).addSelf(l):b[1][n].copy(a[e+h*r]);g+=m;n++});k+=l});this.line.set("attributes",
...
this.matrixZ.set(g[0][2],g[1][2],g[2][2],g[3][2],g[4][2],g[5][2],g[6][2],g[7][2],g[8][2],g[9][2],g[10][2],g[11][2],g[12][2],g[13][2],g[14][2],g[15][2]);var k=this.coefficients;this.matrixX.multiplySelf(k).transpose().multiplySelf(k);this.matrixY.multiplySelf(k).transpose().multiplySelf(k);this.matrixZ.multiplySelf(k).transpose().multiplySelf(k);var k={bezierSurfaceX:this.matrixX,bezierSurfaceY:this.matrixY,bezierSurfaceZ:this.matrixZ},m=c[0][0],l=c[1][0],q=(c[0][1]-m)/(j[0]-1),n=(c[1][1]-l)/(j[1]-
1),r=0;_.loop(j[1],function(){m=c[0][0];_.loop(j[0],function(){a[r].set(m,l,0);m+=q;r++}.bind(this));l+=n}.bind(this));k={uniforms:k,shaders:{position:function(a,b){f?(a.snippet("bezierSurface"+e),a.group(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.next(),a.snippet("mathTransform"),b.viewport(a),a.combine()):(a.snippet("bezierSurface"+e).snippet("mathTransform"),b.viewport(a))}}};this.mesh.set(k);this.line.set(k);this.points.set(k)}});
...
if(+d==d){var s=+d;if(0!=s)var d=0>s?-1:1,s=Math.abs(s),t=Math.pow(10,g-1-Math.floor(Math.log(s)*l)),d=s=d*Math.round(t*s)/t}m||(d=(""+d).replace(/^-/,"\u2013"))}b.content!==d&&(s=b.element.children[0],b.content=d,b.measure=!0,m?(s.innerHTML="\\("+d+"\\)",MathJax.Hub.queue.Push(["Typeset",MathJax.Hub,s])):s.innerHTML=d)});MathBox.Renderable.prototype.adjust.call(this,a)}});
...
a);min=b[0];max=b[1];alpha=this._uniforms.polarAlpha;aspect=this._uniforms.polarAspect;focus=this._uniforms.polarFocus;1==a&&0<alpha&&(max=Math.max(Math.abs(max),Math.abs(min)),min=Math.max(-focus/aspect+0.001,min));return[min,max]},update:function(a){var b=this.get(),c=b.range,d=b.scale,e=b.polar,f=b.fold,j=b.power,b=b.helix,h=1,g=c[0][0],k=c[1][0],m=c[2][0],l=c[0][1]-g,q=c[1][1]-k,c=c[2][1]-m,n=d[0],r=d[1],d=d[2],h=0<l?1:-1,s=Math.abs(q),s=l+(s*h-l)*e,h=Math.abs(s/n/(q/r)),t=[s/(2*n),0,0,g+l/2,
0,q/(2*r),0,k+q/2,0,0,c/(2*d),m+c/2,0,0,0,1];this.transform.set.apply(this.transform,[2*n/s,0,0,-(2*g+l)*n/l,0,2*r/q,0,-(2*k+q)*r/q,0,0,2*d/c,-(2*m+c)*d/c,0,0,0,1]);this.inverse.set.apply(this.inverse,t);this._uniforms.polarAlpha=e;this._uniforms.polarAspect=h;this._uniforms.polarFold=f;this._uniforms.polarPower=j;this._uniforms.polarFocus=0<e?1/e-1:0;this._uniforms.polarHelix=b;MathBox.Viewport.prototype.update.call(this,a)},shader:function(a){a.snippet("polarPower").snippet("cartesianToPolar").snippet("mathToWorld")},
